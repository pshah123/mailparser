...
module.exports = (input, options, callback) => {
    ...
    const skipAttachments = !!options.skipAttachments;
    ...
    let reader = () => {
        ...
        if (data.type === 'attachment') {
            ...
            if (skipAttachments && (
                keepCidLinks || //* if kepecidlinks then skip images
                !(typeof data?.contentType === "string" && data?.contentType?.startsWith('image/')) //* otherwise skip non-images
            )) {
                data.release();
                return reader();
            }
